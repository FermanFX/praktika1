...
// let prodUrl = "http://localhost:3000/singers";
...
//   <div class="mt-1 p-2">
//     <h2 class="text-slate-700">${data.name}</h2>
//     <p class="mt-1 text-sm text-slate-400">${data.country}-lidir</p>

//     <div class="mt-3 flex items-center justify-between">
//       <p class="text-lg font-bold text-blue-500"></p>

//       <i class="fa-regular fa-heart fa-2x favIcon" style="color:red" name="${data.id}"></i>
//       <div
...
axios(prodUrl).then((res) => {
  ...
  datas.forEach((data) => {
    productBox.innerHTML += `
<article
class="card rounded-xl bg-white p-3 shadow-lg hover:shadow-xl hover:transform hover:scale-105 duration-300"
>
<a href="detail.html?id=${data.id}" class="item">
  <div class=" relative flex items-end overflow-hidden rounded-xl">
    <img
    class="cardImg"
      src="${data.image}"
  ...
  <div class="mt-1 p-2">
    <h2 class="prodName text-slate-700">${data.name}</h2>
    <p class="mt-1 text-sm text-slate-400">${data.country}</p>
...
`;
  ...
  for (let icon of favIcons) {
    for (let data of arr) {
      if (icon.getAttribute("name") == data.id) {
        icon.classList.remove("fa-regular");
        icon.classList.add("fa-solid");
      }
    }
    ...
    icon.addEventListener("click", function (e) {
      ...
      if (this.classList.contains("fa-solid")) {
        this.classList.add("fa-regular");
        this.classList.remove("fa-solid");
        arr = arr.filter((data) => data.id != this.getAttribute("name"));
        localStorage.setItem("fav", JSON.stringify(arr));
  ...
  for (let basketBtn of basketBtns) {
    basketBtn.addEventListener("click", function (e) {
      ...
      if (basketArr.find((data) => data.id == this.name)) {
